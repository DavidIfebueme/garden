...
const readIssueInputSchema = z
  .object({
    ...
    comments_page: z
      ...
      .optional()
      .describe(
        'Which bounded comment page to load. Default tail returns newest comments.',
      ),
    comments_limit: z
      .number()
      .int()
    ...
    comments_offset: z
      ...
      .optional()
      .describe(
        'Offset into the selected head/tail page. For tail, offset 20 gets the next older page.',
      ),
  })
  .strict()
...
> {
  ...
  const issueUuid = z.string().uuid().safeParse(args.issueIdOrIdentifier.trim())
...
async function readRunPlan(args: {
  ...
  const stub = AgentDO.get(agentDoId) as unknown as {
    getRunPlan: (input: { runId: string; issueId: string }) => Promise<Array<{
      content: string
      status: 'pending' | 'in_progress' | 'completed'
      activeForm: string
    }> | null>
...
async function listWorkspaceInventoryFromChat(
  ...
  const omittedSections = (
    ['agents', 'members', 'skills', 'connectors'] as const
      ...
      const [
        ...
        includeSkills
          ? db
              ...
              .where(
                and(
                  eq(schema.skillAssignment.workspaceId, identity.workspaceId),
                  eq(
