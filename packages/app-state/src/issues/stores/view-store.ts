...
import type { IssueStatus, IssuePriority } from '@garden/core/types'
import { ALL_STATUSES } from '@garden/core/issues/config/status'
import {
...
export function createIssueViewStore(
  persistKey: string,
...
export const useIssueViewStore = create<IssueViewState>()(
  persist(viewStoreSlice, viewStorePersistOptions('garden_issues_view')),
)
...
type WorkspaceSubscription = (
  callback: (workspaceId: string | undefined) => void,
) => void

/**
 * Wires issue filter state to the app's workspace subscription.
 *
 * The previous optional subscription silently became a no-op when the app
 * layer forgot to pass one. Garden's workspace store is the only intended
 * caller, so the subscription is required and missing wiring fails at compile
 * time instead of leaking filters across workspaces.
 */
export function registerViewStoreForWorkspaceSync(
  store: StoreApi<IssueViewState>,
  subscribeToWorkspace: WorkspaceSubscription,
) {
