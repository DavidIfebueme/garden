...
export const ParsedMimeMessage = Schema.Struct({
  ...
  authoredAt: Schema.NullOr(UtcTimestamp),
...
/** Converts a valid RFC Date header into the canonical UTC wire timestamp. */
const authoredAt = (value: string | undefined): string | null => {
...
export const parseNormalizedMime = Effect.fn('MailMime.parseNormalized')(
  function* (inbound: NormalizedInboundMail) {
    ...
    return yield* decodeParsedMimeMessage({
      ...
      authoredAt: authoredAt(parsed.date),
