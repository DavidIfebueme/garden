...
CREATE TABLE "mail_draft" (
	...
	CONSTRAINT "mail_draft_sender_check" CHECK (("mail_draft"."from_address_id" is not null)::int + ("mail_draft"."from_sync_account_id" is not null)::int = 1),
...
CREATE TABLE "mail_mailbox" (
	...
	"name" text NOT NULL,
	"kind" text NOT NULL,
	"origin" text DEFAULT 'garden_hosted' NOT NULL,
	"status" text DEFAULT 'active' NOT NULL,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	...
	CONSTRAINT "mail_mailbox_origin_check" CHECK ("mail_mailbox"."origin" in ('garden_hosted', 'external_import'))
);
--> statement-breakpoint
...
CREATE TABLE "mail_message" (
	...
	CONSTRAINT "mail_message_outbound_sender_check" CHECK ("mail_message"."source" <> 'outbound' or (("mail_message"."sender_address_id" is not null)::int + ("mail_message"."sender_sync_account_id" is not null)::int = 1))
);
--> statement-breakpoint
...
CREATE UNIQUE INDEX "mail_recipient_message_kind_position_unique" ON "mail_recipient" USING btree ("message_id","kind","position");--> statement-breakpoint
CREATE INDEX "mail_recipient_search_idx" ON "mail_recipient" USING gin (to_tsvector('simple', coalesce("display_name", '') || ' ' || coalesce("address", '')));--> statement-breakpoint
...
CREATE INDEX "mail_sync_run_account_created_idx" ON "mail_sync_run" USING btree ("sync_account_id","created_at");