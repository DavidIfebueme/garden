...
/**
 * Renders a Worker-safe email verification email matching the password reset
 * template's letter-like system. The one-hour lifetime mirrors Better Auth's
 * configured token lifetime; explicit HTML avoids the Node-only react-email
 * rendering path.
 */
export function renderVerificationEmailHtml(props: VerificationEmailProps) {
  const safe = {
    recipientName: escapeHtml(props.recipientName),
    verificationUrl: escapeHtml(props.verificationUrl),
    verificationHref: escapeAttribute(props.verificationUrl),
  }
...
<html lang="en">
  <head>
    ...
    <title>Verify your Garden email</title>
    <style>
            ...
            <tr>
              <td align="center" class="garden-panel" style="background:${colors.vellum};border:1px solid ${colors.hairline};border-radius:18px;padding:40px 38px 32px;">
                ...
                <p style="max-width:390px;margin:16px auto 0;color:${colors.gravel};font-size:15px;line-height:24px;text-align:center;">
                  Hi ${safe.recipientName}, verify your email to finish setting up your Garden account.
                </p>
                <table role="presentation" cellspacing="0" cellpadding="0" border="0" align="center" style="margin:30px auto 34px;">
                  <tr>
                    <td bgcolor="${colors.ink}" style="border:1px solid ${colors.ink};border-radius:10px;">
                      <a href="${safe.verificationHref}" style="display:inline-block;color:${colors.parchment};font-size:14px;font-weight:600;line-height:18px;padding:12px 18px;text-decoration:none;">
                        Verify email
                      </a>
                    </td>
                  </tr>
                ...
                <p style="margin:0;color:${colors.gravel};font-size:13px;line-height:21px;text-align:center;">
                  This link expires in one hour and can only be used once. If you didn’t create this account, you can safely ignore this email.
                </p>
                <p style="max-width:410px;margin:20px auto 0;padding-top:20px;border-top:1px solid ${colors.hairline};color:${colors.gravel};font-size:12px;line-height:19px;text-align:center;">
                  ...
                  <a href="${safe.verificationHref}" style="color:${colors.brand};text-decoration:underline;word-break:break-all;">${safe.verificationUrl}</a>
                </p>
              </td>
            </tr>
            <tr>
