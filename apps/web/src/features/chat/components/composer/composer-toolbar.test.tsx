...
vi.mock("@tiptap/react", () => ({
  useEditorState: ({ editor, selector }: { editor: unknown; selector: (a: { editor: unknown }) => unknown }) =>
    selector({ editor }),
}));
...
describe("ComposerToolbar", () => {
  ...
  it.each(COMMAND_CASES)('"$name" button dispatches $method$args', async ({ name, method, args }) => {
    const editor = makeEditor();
    render(<ComposerToolbar editor={editor} />);
    await userEvent.click(screen.getByRole("button", { name: new RegExp(`^${name}$`, "i") }));
    expect((editor as never as { __calls: unknown[] }).__calls).toContainEqual({ method, args });
  });

  it('"Clear formatting" button dispatches unsetAllMarks then clearNodes', async () => {
    const editor = makeEditor();
    render(<ComposerToolbar editor={editor} />);
    await userEvent.click(screen.getByRole("button", { name: /clear formatting/i }));
    const calls = (editor as never as { __calls: Array<{ method: string }> }).__calls;
    const methods = calls.map((c) => c.method);
    expect(methods).toContain("unsetAllMarks");
    expect(methods).toContain("clearNodes");
    expect(methods.indexOf("unsetAllMarks")).toBeLessThan(methods.indexOf("clearNodes"));
  });
